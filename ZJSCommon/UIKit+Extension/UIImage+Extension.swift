...
extension UIImage {
    ...
    ///   - lineColor: 描边颜色
    /// - Returns: 将图像绘制成圆形图像
    func js_circleImage(size: CGSize, opaque: Bool = true, backColor: UIColor? = UIColor.white, lineColor: UIColor?) -> UIImage? {
        // 1. 计算图片画布的Rect
    ...
    /// 按图片的短边等比缩放的图片大小
    private static func scaleSizeByMinSide(imageSize: CGSize, toSize: CGSize) -> CGSize {
...
extension UIImage {
    ...
    /// - Returns: 缩放后的图像
    func js_scaleImage(maxSize: CGSize) -> UIImage? {
        // 等比缩放的大小
    ...
    ///   - maxSize: 图片存放的最大矩形区域
    /// - Returns: 等比缩放后的大小
    private static func imageScaleSize(imageSize: CGSize, maxSize: CGSize) -> CGSize {
        let imgW = imageSize.width
        let imgH = imageSize.height
...
extension UIImage {
    /// 高斯模糊 建议iOS9以上版本使用
    func js_vagueImage(blur: CGFloat) -> UIImage? {
        var drawImage = self
        ...
        if self.size.width > 720 {
            ...
            drawImage = compressedPicture(size: rect.size)
        }
        ...
        /* 在iOS8各系统版本下，Swift中使用 “CIContext(options: nil)” 初始化CIContext，在模拟器运行时直接crash，错误如下：
         reason: '-[CIContext initWithOptions:]: unrecognized selector sent to instance'
         iOS9.0及以上系统这样初始化没问题 */
    ...
    /// View截图
    ///
    /// - Parameter currentView: 要截图的view
    /// - Returns: 返回Image
    static func currentScreenImage(currentView: UIView) -> UIImage {
